...
public class AutoLeft extends SequentialCommandGroup {
    ...
    addCommands(
     ...
     //       new setShootModeOn(m_ballShooter),
            new driveFeet(2, 0, m_driveTrain).withTimeout(3.5)
        ),
        

        new autoTurn(21, m_driveTrain).withTimeout(2),
        new turn2LimeLight(m_driveTrain).withTimeout(3),
        new autonimousShoot(m_ballShooter).withTimeout(4),
        //new teleopAutoShootCMD(m_ballShooter).withTimeout(4),
        new autoTurn(-21, m_driveTrain).withTimeout(2),
        new extendSolenoid(m_ballAcquisition).withTimeout(1),

        new ParallelRaceGroup(
            new startAcquireMotor(m_ballAcquisition),
            new driveFeet(13, 0, m_driveTrain).withTimeout(5)
        ),
        new driveFeet(-13, 0, m_driveTrain).withTimeout(5)   
        );
    }
